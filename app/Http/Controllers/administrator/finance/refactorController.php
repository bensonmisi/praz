...
class refactorController extends Controller
{
     public function index(){
        $invoice = online_invoice::with('currency')->wherestatus('PAID')->get();
        ...
        foreach ($grouped as $key => $value) {
             ...
             $status="";
               foreach ($value as $val) {
                ...
                if(($totalinvoiced-$totalreceipted) !=0)
                {
                 $array[] = array("invoicenumber"=>$key,"totalinvoice"=>$totalinvoiced,"totalpaid"=>$totalreceipted);
                }
                                 ...
                                 if($total_invoiced <=$total && $total != $total_receipted){
                                   ...
                                   receipt::whereinvoicenumber($key)->delete();
                                    foreach ($online as $keys => $val) {
                                        ...
                                        receipt::firstOrCreate(['source_id'=>$val->id],['invoicenumber'=>$key,'receiptnumber'=>$receiptnumber,'company_id'=>$val->company_id,'method'=>$val->mode,'currency'=>$val->invoice->currency->name,'amount'=>$val->amount,'user_id'=>$val->invoice->user_id]);
     ...
     function checkOnlinepayment($invoicenumber){
