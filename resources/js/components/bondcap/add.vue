<template>
    <div>
        ...
        <v-dialog v-model="addModel" max-width="600">
            ...
            <v-card>
                <v-card-title>
                    <div>New Bond Cap</div>
                    <v-spacer></v-spacer>
                    <v-btn icon @click="addModel=false"><v-icon>mdi-close</v-icon></v-btn>
                </v-card-title>
                <v-card-text>
                    <v-text-field outlined label="Value" v-model="name" :rules="nameRule"> </v-text-field>
                </v-card-text>
                <v-card-actions>
...
export default {
    ...
    data(){
        ...
        save(){
                ...
                cap.add(formData).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "Bid Bond Cap successfully add"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
