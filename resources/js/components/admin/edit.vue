        ...
        <v-dialog v-model="addModel" max-width="600">
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-select outlined label="Status" v-model="status" :items="statuslist"  :rules="statusRule"></v-select>
                     <v-checkbox v-model="resetpwd" label="Reset Password to default"></v-checkbox>
         
                </v-card-text>
                <v-card-actions>
                    <v-btn outlined rounded class="red" @click="addModel=false">Cancel</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn outlined rounded class="green" @click="updateUser">Submit</v-btn>
                </v-card-actions>
            </v-card>
            </v-form>
        </v-dialog>
    ...
    data(){
        ...
        updateUser(){
            if(this.$refs.form.validate()){
                ...
                admin.updateUser(formData,this.user.id).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "User successfully Update"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
