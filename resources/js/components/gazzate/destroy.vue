        ...
        <v-dialog v-model="addModel" max-width="600">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn outlined rounded class="green" @click="destroy">Delete</v-btn>
                </v-card-actions>
            </v-card>
...
export default {
    ...
    data(){
        ...
        destroy(){         
                this.loading = true
                api.destroy(this.data.id).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "Gazzate successfully Destroy"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
