<template>
    <div>
        ...
        <v-dialog v-model="addModel" max-width="600">
            ...
            <v-card>
                <v-card-title>
                    <div>Delete Threshold</div>
                    <v-spacer></v-spacer>
                    <v-btn icon @click="addModel=false"><v-icon>mdi-close</v-icon></v-btn>
                </v-card-title>
                <v-card-text>
                    <v-text-field outlined label="value" readonly v-model="value"> </v-text-field>
                    <v-text-field outlined label="pclass" readonly v-model="pclass"> </v-text-field>
                    <v-text-field outlined label="pclassification" readonly v-model="pclassification"> </v-text-field>
                </v-card-text>
                <v-card-actions>
...
export default {
    ...
    data(){
        ...
        destroy(){           
                this.loading = true
                api.destroy(this.threshold.id).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "Procurement Threshold successfully deleted"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
