<template>
    <div>
        ...
        <v-dialog v-model="addModel" max-width="600">
            <v-card>
                <v-card-title>
                    <div>Delete Establishment Fees</div>
                    <v-spacer></v-spacer>
                    <v-btn icon @click="addModel=false"><v-icon>mdi-close</v-icon></v-btn>
                </v-card-title>
                <v-card-text>
                             <v-text-field outlined label="Lower" v-model="lower" readonly></v-text-field>
                     <v-text-field outlined label="Upper" v-model="upper" readonly></v-text-field> 
                      <v-select outlined label="Validity Period(days)" v-model="validity" :items="validitylist" readonly></v-select>
                               
                    <v-text-field outlined label="Value" v-model="value" readonly> </v-text-field>
                </v-card-text>
                <v-card-actions>
