        ...
        <v-dialog v-model="addModel" max-width="600">
            ...
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn outlined rounded class="green" @click="saveModule">Submit</v-btn>
                </v-card-actions>
            </v-card>
            </v-form>
        </v-dialog>
...
<script>
import * as role from '../../services/module_service'
export default {
    name:'addRole',
    data(){
        return{
        ...
        saveModule(){
            if(this.$refs.form.validate()){
                ...
                role.addModule(formData).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "Module successfully add"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
