        ...
        <v-dialog v-model="addModel" max-width="600">
            ...
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn outlined rounded class="green" @click="updateType">Submit</v-btn>
                </v-card-actions>
            </v-card>
            </v-form>
        </v-dialog>
    ...
    data(){
        ...
        updateType(){
            if(this.$refs.form.validate()){
                ...
                type.updateAccounttype(formData,this.type.id).then(response=>{
                    this.loading = false
                    this.snackbar = true
                    this.message = "Account successfully Update"
                    this.color="green"
                    this.addModel = false
                    this.$emit('reload')
                }).catch(error=>{
