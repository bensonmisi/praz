 ...
 @include('partials.message')  
   ...
   <form action="{{route('loginSubmit')}}" method="POST">    
    <div class="w-full px-3 mb-6">
       <input class="appearance-none border @error('email') border-red-600 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="name" type="text" wire:model.lazy="email" placeholder="Email">
      @error('email')
    ...
    <div class="w-full  px-3 mb-6">
      ...
      <input class="appearance-none border @error('password') border-red-600 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="password" type="password" wire:model.lazy="password" placeholder="Password">
      @error('password')
         <span class="text-red-600" role="alert">
          <strong>{{ $message }}</strong>
         </span>
    ...
    <div class="mt-2 px-3">
      ...
      @if(env('GOOGLE_RECAPTCHA_KEY'))
       <input type="hidden" class="g-recaptcha"  data-sitekey="{{env('GOOGLE_RECAPTCHA_KEY')}}" wire:model="g_recaptcha_response" name="g_recaptcha_response"/>
