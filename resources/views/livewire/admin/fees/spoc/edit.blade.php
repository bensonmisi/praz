...
<div class="bg-gray-200 px-3 py-4 rounded-md flex justify-start items-center">
       <div class="text-indigo-600 px-3 py-3 mr-2"><a href="{{route('admin.home')}}">Dashboard</a></div><div class="mr-2">/</div><div class="text-indigo-600 px-3 py-3 mr-2"><a href="{{route('admin.fees.spoc')}}">SPOC Fees</a></div><div class="mr-2">/</div><div class="mr-2">Update Fee</div>
</div>
<div class="w-1/2 mx-auto bg-white px-3 py-5 rounded-sm shadow-md border border-solid m-1 border-gray-200">
     ...
     <div class="w-full p-3 mb-6 md:mb-0">
      ...
      <select class="appearance-none border @error('threshold') border-red-600 @enderror rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="threshold" wire:model.lazy="threshold">
      <option></option>
                             @forelse($thresholds as $curr)
                                <option value="{{$curr->id}}">{{$curr->value}}:{{$curr->classification->name}}</option>
                             @empty
    ...
    Fee
      ...
      @error('value')
       ...
       @enderror
        ...
        <a href="{{route('admin.fees.spoc')}}" class="px-3 py-3 bg-red-600 rounded-md hover:bg-red-400 text-white">Cancel</a>
